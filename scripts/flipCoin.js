...
module.exports = robot => {
  robot.hear(/!flip(?: ([0-9]*))?$/i, res => {
    const { user } = res.message;
    const [, _flips = 1] = res.match;
    ...
    const results = Array.from(rng, number => {
      ...
      if (flips > 10) {
        ...
        if (face !== currentSequence.face) {
          if (currentSequence.length >= 2)
            details.sequences.push(currentSequence);
    ...
    const plural = (count, s, p) => (count > 1 ? p : s);

    const reply = [results.join(' ')];
    const timestamp = Math.floor(Date.now() / 1000);

    const attachment = {
      fallback: `Results for ${flips} Coin ${plural(
        flips,
        'Flip',
        'Flips'
      )} - Heads: ${details.heads} - Tails: ${details.tails}`,
      author_name: 'Coin Flipper',
      title: `Results for ${flips} Coin ${plural(flips, 'Flip', 'Flips')}`,
      pretext: `*<@${user.id}> flipped ${plural(
        flips,
        'a coin',
        `${flips} coins`
      )}*`,
